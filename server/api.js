...
app.post("/data/:page", async (req, res) => {
...
app.post("/getCount", async (req, res) => {
  const allCount = await model.openkatok.estimatedDocumentCount()
  const validCount = await model.openkatok
    .find({ valid: true })
    .estimatedDocumentCount()
  console.log(allCount, validCount)
  res.json({ allCount: allCount, validCount: validCount })
})
